...
double t_invcdf_Q(double args[]) {
  return gsl_cdf_tdist_Qinv(args[0], args[1]);
}
 ...
 * gsl_cdf_weibull_P(args[0],           args[1],   args[2]);  } */
...
double binomial_P(double args[]) {
  return gsl_cdf_binomial_P(args[0], args[1], args[2]);
}
double binomial_Q(double args[]) {
  return gsl_cdf_binomial_Q(args[0], args[1], args[2]);
}
